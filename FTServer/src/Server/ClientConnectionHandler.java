...
public class ClientConnectionHandler implements Runnable{
    ...
    private void loginRequest(CommunicationProtocol request){
        ...
        synchronized(clientesLigados){
            for(Client client : clientesLigados){
                if(client.getNome().equalsIgnoreCase(clientForLogin.getNome())){
                    response.setCodeProtocol("ERROR");
                    response.setMessage("Nome duplicado");
                    duplicateFlag = true;
                }
            }
        }
        ...
        if(duplicateFlag){
            ...
            CommunicationProtocol messageLogin = new CommunicationProtocol("NOTIFICATION");
            messageLogin.setMessage(tempoAtualStr + "  -  " + "O utilizador " + clientForLogin.getNome() + " fez login");
            notificarGrupoMulticast(messageLogin);
        } 
    }
    ...
    private void updateFilesRequest(CommunicationProtocol request){
        ...
        synchronized(clientesLigados){
            for(Client client : clientesLigados){
                if(client.getNome().equals(request.getNameSender())){
        ...
        response.setCodeProtocol("OK");
        response.setMessage("Ficheiros atualizados com sucesso");
        sendResponse();
    ...
    private void logoutRequest(CommunicationProtocol request){
        //Obter o nome do quem fez o request
        ...
        synchronized(clientesLigados){
            ...
            while(iterator.hasNext()){
                ...
                if(client.getNome().equals(nameSender)){
                    iterator.remove();
                }
            }
        }
